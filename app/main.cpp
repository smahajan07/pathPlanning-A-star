...
class Node{
    ...
    public:
        ...
        void addNode(Node& newNode,const int& x, const int& y, int& id,int pNum,const int& dFlag);
        ...
        bool checkDuplicate(tuple<int, int> nodePos,const int& idx);
};
...
//queue<int> levelQ;
priority_queue<Node, vector<Node>,compareCost> levelQ;
queue<int> closedQ;

tuple<int, int> goalNode(205,40); // Change value for Goal Node
tuple<int, int> finalPath[400];
int pathNodes=0;
...
void createSpace();
double heuristicFn(const int& x, const int& y);
...
void Node::actionMoveNorth(const int& idx,int x, int y, const int& y_old){
    ...
    if(y<150 && y!=y_old){
        bool check1 = checkDuplicate(make_tuple(x,y),idx);
        bool check2 = checkObstacle(make_tuple(x,y));
        if(check1 && check2){
            totalNodesCreated++;
            nodesInfo[totalNodesCreated-1].addNode(nodesInfo[totalNodesCreated-1], x, y, totalNodesCreated, idx,0);
...
void Node::actionMoveNorthEast(const int& idx,int x, int y, const int& x_old, const int& y_old){
    ...
    if(y<150 && x<250 && y!=y_old && x!=x_old){
        ...
        if(check1 && check2){
            totalNodesCreated++;
            nodesInfo[totalNodesCreated-1].addNode(nodesInfo[totalNodesCreated-1], x, y, totalNodesCreated, idx,1);
...
void Node::addNode(Node& newNode,const int& x, const int& y, int& id, int pNum,const int& dFlag){
    newNode.currNodePos = make_tuple(x,y);
    newNode.idNum = id;
    newNode.parentNum = pNum;
    dFlag?newNode.tCost = 1.4:newNode.tCost = 1;

    newNode.gCost = nodesInfo[pNum-1].gCost + newNode.tCost;

    newNode.cost = newNode.gCost + heuristicFn(x,y);
    levelQ.push(newNode);
}

double heuristicFn(const int& x, const int& y){

  int goalX = get<0>(goalNode);
  int goalY = get<1>(goalNode);

  return sqrt(pow((goalX-x),2) + pow((goalY-y),2));

}

bool Node::checkDuplicate(tuple<int, int> nodePos,const int& idx){
    for(int i=0;i<totalNodesCreated;++i){
        if(nodePos == nodesInfo[i].currNodePos){
          int p = nodesInfo[i].parentNum;
          if(nodesInfo[idx-1].cost < nodesInfo[p-1].cost)
            nodesInfo[i].parentNum = idx;
          return false;
        }
    }
    return true;
...
void findPathAstar(tuple<int, int> nodePos){
...
int main()
{
    ...
    if(beginFlag){
        ...
        int rootID = 1;
        nodesInfo[ROOT].addNode(nodesInfo[ROOT],startX, startY, rootID, 0,0);
        tuple<int, int> prevNodePosRoot(-1,-1);
        nodesInfo[ROOT].prevNodePos = prevNodePosRoot;
        nodesInfo[ROOT].cost = 0;
        nodesInfo[ROOT].gCost = 0;
        levelQ.push(nodesInfo[ROOT]);
